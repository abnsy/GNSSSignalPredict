...
results = pd.DataFrame ( { 'SVid' : df.Svid.to_numpy ( ) , 'ContellationType' : df.ConstellationType.to_numpy ( ) ,
                           'TDoA_[ms]' : np.round ( p_ms , decimals = 3 ) ,
                           'Pseudoranges_[m]' : p_m.astype ( dtype = 'int' ) } )
